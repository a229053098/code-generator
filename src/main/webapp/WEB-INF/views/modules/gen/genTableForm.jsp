		...
		<c:otherwise>
			<form:form id="inputForm" modelAttribute="genTable" action="${ctx}/gen/genTable/save" method="post" class="form-horizontal">
				...
				<fieldset>
					...
					<div class="control-group">
						<label class="control-label">导出Excel:</label>
						<div class="controls">
							<form:radiobuttons path="exportExcel" items="${fns:getDictList('yes_no')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
							<span class="help-inline" style="color: #9c0001">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;注意：选是，则列表必须至少展示一个字段</span>
						</div>
					</div>
					<div class="control-group">
					...
					<div class="control-group">
						...
						<div class="controls">
							<form:input path="defaultSortField" htmlEscape="false" maxlength="200" />
							<span class="help-inline">例如：创建时间倒序填 create_time desc</span>
						</div>
					</div>
					<legend>字段列表</legend>
					<div class="control-group">
						<table id="contentTable" class="table table-striped table-bordered table-condensed">
							<thead>
								<tr>
									...
									<th title="">关联的表名 <i class="icon-question-sign"></i></th>
									<th title="">关联的实体名 <i class="icon-question-sign"></i></th>
									<th title="">关联字段|字段名|属性名 <i class="icon-question-sign"></i></th>
									<th title="是否是数据库主键">主键</th><th title="字段是否可为空值，不可为空字段自动进行空值验证">可空</th>
									<%--<th title="选中后该字段被加入到insert语句里">插入</th>--%>
									<th title="选中后该字段被加入到update语句里">编辑【编辑页】</th><th title="选中后该字段被加入到查询列表里">列表</th>
									<th title="选中后该字段被加入到查询条件里">查询【列表筛选&WHERE条件】</th><th title="该字段为查询字段时的查询匹配放松">查询匹配方式</th>
							...
							<c:forEach items="${genTable.columnList}" var="column" varStatus="vs">
								<tr${column.delFlag eq '1'?' class="error" title="已删除的列，保存之后消失！"':''}>
									...
									<td>
										<input type="text" name="columnList[${vs.index}].referenceTable" value="${column.referenceTable}" maxlength="200" class="input-small"/>
									</td>
									<td>
										<input type="text" name="columnList[${vs.index}].referenceEntity" value="${column.referenceEntity}" maxlength="200" class="input-small"/>
									</td>
									<td>
										<input type="text" name="columnList[${vs.index}].referenceField" value="${column.referenceField}" maxlength="200" class="input-small"/>
									</td>
									<td>
